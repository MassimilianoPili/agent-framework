...
/**
 * Unit tests for {@link ModelPredictiveControlService}.
 *
 * <p>Verifies MPC scheduling computation with multiple profiles, insufficient data,
 * and single-profile degenerate cases.</p>
 */
@ExtendWith(MockitoExtension.class)
class ModelPredictiveControlServiceTest {

    @Mock private TaskOutcomeRepository taskOutcomeRepository;
    @Mock private WorkerProfileRegistry profileRegistry;

    private ModelPredictiveControlService service;

    @BeforeEach
    void setUp() {
        service = new ModelPredictiveControlService(taskOutcomeRepository, profileRegistry);
        ReflectionTestUtils.setField(service, "horizon", 3);
     ...
     */
    private Object[] makeRow(double actualReward, double gpMu) {
    ...
    @Test
    @DisplayName("computeSchedule with multiple profiles returns MPC optimization result")
    void computeSchedule_withMultipleProfiles_returnsMpcResult() {
        when(profileRegistry.profilesForWorkerType(WorkerType.BE))
                .thenReturn(List.of("be-java", "be-go", "be-rust"));

        // be-java: mean reward = 0.8, gpMu = 0.7
        List<Object[]> javaData = new ArrayList<>();
        for (int i = 0; i < 10; i++) javaData.add(makeRow(0.8, 0.7));
        when(taskOutcomeRepository.findTrainingDataRaw("BE", "be-java", 100))
                .thenReturn(javaData);

        // be-go: mean reward = 0.6, gpMu = 0.5
        List<Object[]> goData = new ArrayList<>();
        for (int i = 0; i < 10; i++) goData.add(makeRow(0.6, 0.5));
        when(taskOutcomeRepository.findTrainingDataRaw("BE", "be-go", 100))
                .thenReturn(goData);

        // be-rust: mean reward = 0.7, gpMu = 0.6
        List<Object[]> rustData = new ArrayList<>();
        for (int i = 0; i < 10; i++) rustData.add(makeRow(0.7, 0.6));
        when(taskOutcomeRepository.findTrainingDataRaw("BE", "be-rust", 100))
                .thenReturn(rustData);

        MpcScheduleReport report = service.computeSchedule("BE");

        assertThat(report).isNotNull();
        assertThat(report.workerType()).isEqualTo("BE");
        assertThat(report.profilesEvaluated()).isEqualTo(3);
        assertThat(report.profileRewards()).hasSize(3);
        assertThat(report.mpcResult()).isNotNull();
        assertThat(report.mpcResult().recommendedAction()).isNotNull();
        assertThat(report.mpcResult().trajectoriesEvaluated()).isGreaterThan(0);
    }

    @Test
    @DisplayName("computeSchedule with insufficient data returns null")
    void computeSchedule_insufficientData_returnsNull() {
        when(profileRegistry.profilesForWorkerType(WorkerType.BE))
                .thenReturn(List.of("be-java", "be-go"));

        // Both profiles have < MIN_OUTCOMES (5)
        List<Object[]> tooFew = new ArrayList<>();
        for (int i = 0; i < 3; i++) tooFew.add(makeRow(0.5, 0.5));
        when(taskOutcomeRepository.findTrainingDataRaw("BE", "be-java", 100))
                .thenReturn(tooFew);
        when(taskOutcomeRepository.findTrainingDataRaw("BE", "be-go", 100))
                .thenReturn(tooFew);
    ...
    @Test
    @DisplayName("computeSchedule with single profile returns greedy action")
    void computeSchedule_singleProfile_returnsGreedyAction() {
        when(profileRegistry.profilesForWorkerType(WorkerType.BE))
                .thenReturn(List.of("be-java", "be-go"));
        ...
        assertThat(report.mpcResult().recommendedAction()).isNotNull();
        assertThat(report.mpcResult().recommendedAction().taskIndex()).isEqualTo(0);
    }
}
