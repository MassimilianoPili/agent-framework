...
|-----|-------------|----------------|
| **B1** ACK after commit | `TransactionSynchronization.afterCommit()` — XACK solo dopo commit DB | `AgentResultConsumer.java`, `RedisStreamListenerContainer.java` |
...
| **B4** AutoRetryScheduler fix | `REQUIRES_NEW` per item — un retry fallito non causa rollback di tutti | `AutoRetryScheduler.java`, `RetryTransactionService.java` (NEW) |
| **B5** Missing context propagation | CM task fallito → propaga FAILED al task padre | `OrchestrationService.java` |
...
**Invarianti**: `gp.enabled: false` → budget statico come prima. Cold start → budget = base.

---

### Sessione 11 — Council Taste Profile (#13) + Modello LLM per Task (#20) ✅ COMPLETATA

> **Stato**: completata il 2026-03-08. Test: 632 → 888 (+256). BUILD SUCCESS.
...
come `defaultOptions()` sul `ChatClient` — trasparente ai worker.
