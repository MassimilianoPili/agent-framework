...
public class BeElixirWorker extends AbstractWorker {

    private static final Logger log = LoggerFactory.getLogger(BeElixirWorker.class);
    ...
    protected String systemPromptFile() {
        return ".claude/agents/be-elixir/SKILL.md";
    }
    ...
    protected String execute(AgentContext context, ChatClient chatClient)
        ...
        log.info("[BE-Elixir] Executing task '{}' with {} dependency results",
        ...
        try {
            ...
            log.info("[BE-Elixir] Task '{}' completed, response length: {} chars",
                     context.taskKey(), response != null ? response.length() : 0);
            return response;

        } catch (Exception e) {
            throw new WorkerExecutionException(
                "BE-Elixir worker execution failed for task " + context.taskKey(), e);
        }
    }
}
