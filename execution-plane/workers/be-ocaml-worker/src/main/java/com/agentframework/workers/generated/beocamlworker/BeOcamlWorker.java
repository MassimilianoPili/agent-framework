...
public class BeOcamlWorker extends AbstractWorker {

    private static final Logger log = LoggerFactory.getLogger(BeOcamlWorker.class);
    ...
    protected String systemPromptFile() {
        return ".claude/agents/be-ocaml/SKILL.md";
    }
    ...
    protected String execute(AgentContext context, ChatClient chatClient)
        ...
        log.info("[BE-Ocaml] Executing task '{}' with {} dependency results",
        ...
        try {
            ...
            log.info("[BE-Ocaml] Task '{}' completed, response length: {} chars",
                     context.taskKey(), response != null ? response.length() : 0);
            return response;

        } catch (Exception e) {
            throw new WorkerExecutionException(
                "BE-Ocaml worker execution failed for task " + context.taskKey(), e);
        }
    }
}
