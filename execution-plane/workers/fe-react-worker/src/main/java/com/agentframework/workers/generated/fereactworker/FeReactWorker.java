...
public class FeReactWorker extends AbstractWorker {

    private static final Logger log = LoggerFactory.getLogger(FeReactWorker.class);
    ...
    protected String execute(AgentContext context, ChatClient chatClient)
        ...
        log.info("[FE-React] Executing task '{}' with {} dependency results",
        ...
        try {
            ...
            log.info("[FE-React] Task '{}' completed, response length: {} chars",
                     context.taskKey(), response != null ? response.length() : 0);
            return response;

        } catch (Exception e) {
            throw new WorkerExecutionException(
                "FE-React worker execution failed for task " + context.taskKey(), e);
        }
    }
}
