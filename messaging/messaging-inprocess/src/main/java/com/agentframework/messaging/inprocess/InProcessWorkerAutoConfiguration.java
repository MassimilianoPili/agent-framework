 ...
 *
 * <p>Active when {@code messaging.provider=in-process} AND the {@link InProcessMessageBroker}
 * bean exists. This replaces {@code WorkerAutoConfiguration} (which handles the single-worker
 * case) with a multi-worker setup where all 46+ workers share the same JVM.
 *
 * <p>Responsibilities:
 * <ul>
